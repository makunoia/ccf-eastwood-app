...
/**
 * Cluster shared registration form (CCF-132): one public form for an "Event
 * Day". The person registers once, ticks which of the day's events they're
 * attending, and the submission fans out one registration per selected event.
 */
...
export default async function ClusterRegisterPage({
  ...
  // Walk-in mode: the cluster check-in board sends people here when a lookup
  // finds no registration. Staff-supervised — exempt from the open/close rules.
  const walkIn = checkin
    ? {
        occurrenceId: null,
