 ...
 * Eastwood), or `SmallGroup.parentSatellite` (a leader elsewhere). The two
 * cannot both be true, so joining a group here supersedes the satellite.
 *
 ...
 * write. A no-op for the overwhelming majority of people, who lead nothing.
 */
export async function clearUpwardSatelliteOnConfirm(
  ...
  await tx.smallGroup.updateMany({
    where: { leaderId: { in: [...memberIds] }, parentSatellite: { not: null } },
