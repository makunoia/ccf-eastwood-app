...
**The step comes after the DGroup steps**, not before: `sg-profile` is where someone can supply gender and life stage for the first time, and those are the two answers the suggestion turns on. It is skippable, never assigns on skip, and is suppressed entirely when `autoAssignBreakout` is on — the same "replaces rather than sits beside" rule every other surface applies. The facilitator gate is on, matching the door; facilitators check in through the same kiosk on the volunteer lane, so it opens on its own.
