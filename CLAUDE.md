...
**Not on DataTable, on purpose:** the bus manifest print surface, the cluster dashboard's per-event matrix, the event dashboard chart widget, the import wizard previews, the assistant's markdown/tool renderers, and the expanded detail panel inside the Volunteers table. None of them is a person list, and a column picker on a print manifest is noise.
